...
class GameController
  ...
  def select_players(player1, player2)
    if player1 == player2 || !player1.between?(1, @players.length) || !player2.between?(1, @players.length)
      puts "Invalid player choices (#{player1},#{player2})"
    else
      @active_player_one[:player] = @players[player1 - 1]
      @active_player_two[:player] = @players[player2 - 1]
  ...
  def check_win
    ...
    # Tie
    return unless @game_board.board.flatten.map(&:mark).uniq.sort == %w[O X]
